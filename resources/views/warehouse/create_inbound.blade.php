...
@section('top_bar_left')
    <a href="{{ route('warehouse.inbound') }}" class="btn btn-sm btn-white border fw-bold text-secondary me-3"><i class="fa-solid fa-arrow-left"></i> Kembali</a>
    <x-breadcrumb :links="['Warehouse' => '#', 'Barang Masuk' => route('warehouse.inbound'), 'Input Penerimaan' => null]" />
@endsection
...
<div class="container-fluid mx-auto mt-4 mb-5" style="max-width: 1100px;">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4 class="fw-bold mb-0 text-dark">Input Penerimaan Barang (Inbound)</h4>
    </div>
    ...
    <ul class="nav nav-pills mb-4 bg-white p-2 rounded border shadow-sm flex-nowrap overflow-auto">
        <li class="nav-item">
            <a class="nav-link fw-bold px-4 {{ $tab == 'pembelian' ? 'active' : 'text-muted' }}" href="{{ route('warehouse.inbound.create', ['tab' => 'pembelian']) }}"><i class="fa-solid fa-truck-ramp-box me-1"></i> Pembelian (PO)</a>
        </li>
        <li class="nav-item">
            <a class="nav-link fw-bold px-4 {{ $tab == 'transfer_masuk' ? 'active' : 'text-muted' }}" href="{{ route('warehouse.inbound.create', ['tab' => 'transfer_masuk']) }}"><i class="fa-solid fa-plus-circle me-1"></i> Transfer Masuk</a>
        </li>
        <li class="nav-item">
            <a class="nav-link fw-bold px-4 {{ $tab == 'retur_penjualan' ? 'active' : 'text-muted' }}" href="{{ route('warehouse.inbound.create', ['tab' => 'retur_penjualan']) }}"><i class="fa-solid fa-arrow-rotate-left me-1"></i> Retur Penjualan</a>
        </li>
    </ul>
    ...
    @if($tab == 'pembelian')
        <form action="#" method="POST" id="form-pembelian">
            ...
            <div class="card border-0 shadow-sm mb-3">
                <div class="card-header bg-white py-3"><h6 class="fw-bold m-0 text-primary">Terima Barang dari Pesanan Pembelian</h6></div>
                <div class="card-body row g-3">
                    <div class="col-md-6">
                        <label class="form-label fw-bold small text-muted">Pilih Supplier / Vendor *</label>
                    ...
                    <div class="col-md-6">
                        <label class="form-label fw-bold small text-muted">Pilih Purchase Order (PO) *</label>
                        <select id="po_select" class="form-select select2" required disabled>
                            <option value="">-- Pilih Supplier Dahulu --</option>
                        </select>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label fw-bold small text-muted">No. Surat Jalan / Tagihan *</label>
                        <input type="text" name="bill_number" class="form-control fw-bold" placeholder="Misal: SJ-001 / INV-001" required>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label fw-bold small text-muted">Tgl Terima / Masuk *</label>
                        <input type="date" name="receive_date" class="form-control" value="{{ date('Y-m-d') }}" required>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label fw-bold small text-muted">Jatuh Tempo Bayar (Opsional)</label>
                    ...
                    <table class="table table-hover align-middle mb-0" style="font-size: 0.85rem;">
                        <thead class="table-light text-center">
                            <tr>
                                ...
                                <th width="20%">Terima Sekarang</th>
                            </tr>
                        </thead>
                        <tbody id="po-items-body">
                            <tr><td colspan="5" class="text-center py-4 text-muted">Pilih PO untuk memuat barang.</td></tr>
                        </tbody>
                    </table>
                </div>
                <div class="card-footer bg-white border-top-0 pb-4">
                    <button type="submit" class="btn btn-success fw-bold px-4" id="btn-submit-pembelian" disabled><i class="fa-solid fa-save me-1"></i> Proses Penerimaan & Jurnal</button>
                </div>
            </div>
        </form>
    ...
    @elseif($tab == 'transfer_masuk')
        <form action="{{ route('warehouse.inbound.store') }}" method="POST">
            @csrf
            <div class="card border-0 shadow-sm mb-3">
                <div class="card-header bg-white py-3"><h6 class="fw-bold m-0 text-success">Transfer Masuk Manual</h6></div>
                <div class="card-body row g-3">
                    <div class="col-md-3"><label class="form-label fw-bold small text-muted">No. Referensi *</label><input type="text" name="evidence_number" class="form-control fw-bold" value="{{ $autoNumberInbound }}" required></div>
                    ...
                    <div class="col-md-6"><label class="form-label fw-bold small text-muted">Keterangan / Alasan</label><input type="text" name="description" class="form-control" placeholder="Contoh: Stok Opname / Pindah Gudang" required></div>
                    <div class="col-md-12">
                        <label class="form-label fw-bold small text-muted">Akun Penyeimbang Jurnal (KREDIT) *</label>
            ...
            <div class="card border-0 shadow-sm">
                <table class="table table-bordered align-middle mb-0" style="font-size: 0.85rem;">
                    <thead class="table-light text-center">
                        <tr><th width="50%">Pilih Barang</th><th width="20%">Qty Masuk</th><th width="25%">HPP Baru per Unit (Rp)</th><th width="5%"></th></tr>
                    </thead>
                ...
                <div class="card-footer bg-white p-3 d-flex justify-content-between align-items-center">
                    <button type="button" class="btn btn-sm btn-outline-primary fw-bold" onclick="tambahBaris('inbound')">+ Tambah Barang</button>
                    <button type="submit" class="btn btn-success fw-bold px-4"><i class="fa-solid fa-save me-1"></i> Simpan Data</button>
                </div>
            </div>
        </form>
    ...
    @elseif($tab == 'retur_penjualan')
        <form action="{{ route('sales-returns.store') }}" method="POST">
            @csrf
            <div class="card border-0 shadow-sm mb-3">
                <div class="card-header bg-white py-3"><h6 class="fw-bold m-0 text-danger">Terima Retur dari Pelanggan</h6></div>
                <div class="card-body row g-3">
                    <div class="col-md-4">
                    ...
                    <div class="col-md-4">
                        <label class="form-label fw-bold small text-muted">Faktur Penjualan Asal <span class="text-danger">*</span></label>
                    ...
                    <table class="table table-hover align-middle mb-0" style="font-size: 0.85rem;">
                        <thead class="table-dark text-uppercase">
                            <tr>
                                ...
                                <th width="15%">Qty Faktur</th>
                                <th width="15%">Qty Diretur</th>
                            </tr>
                        </thead>
                        <tbody id="items-body-retur">
                            <tr><td colspan="5" class="text-center py-4 text-muted">Pilih Faktur untuk memuat barang.</td></tr>
                        </tbody>
                    </table>
                </div>
                <div class="card-footer bg-white p-3 text-end">
                    <button type="submit" class="btn btn-primary fw-bold px-4" id="btn-submit-retur" disabled><i class="fa-solid fa-save me-1"></i> Buat Dokumen Retur</button>
                </div>
            </div>
        </form>
    @endif
...
<script>
    $(document).ready(function() {
        ...
        $('#supplier_select').on('change', function() {
            ...
            poSelect.html('<option value="">Loading...</option>').prop('disabled', true);
            ...
            if(supplier) {
                ...
                poSelect.html('<option value="">-- Pilih Supplier Dahulu --</option>').prop('disabled', true);
                $('#po-items-body').html('<tr><td colspan="5" class="text-center py-4 text-muted">Pilih PO untuk memuat barang.</td></tr>');
        ...
        $('#po_select').on('change', function() {
            ...
            if(!poId) {
                tbody.html('<tr><td colspan="5" class="text-center py-4 text-muted">Pilih PO untuk memuat barang.</td></tr>');
                $('#btn-submit-pembelian').prop('disabled', true);
                return;
            }

            form.attr('action', `/purchase-order/${poId}/receive`);
            tbody.html('<tr><td colspan="5" class="text-center py-4"><div class="spinner-border spinner-border-sm text-primary"></div> Memuat Barang...</td></tr>');
        ...
        $('#select-invoice').change(function() {
            ...
            if (!invId) {
                tbody.html('<tr><td colspan="5" class="text-center py-4 text-muted">Pilih Faktur untuk memuat barang.</td></tr>');
                $('#input-contact').val('');
                $('#btn-submit-retur').prop('disabled', true);
                return;
            }

            tbody.html('<tr><td colspan="5" class="text-center py-4"><div class="spinner-border spinner-border-sm text-primary"></div> Memuat...</td></tr>');

            $.get('{{ url("sales-returns/get-invoice-items") }}/' + invId, function(res) {
                $('#input-contact').val(res.contact_name);
                tbody.empty();
                if (!res.items || res.items.length === 0) {
                    tbody.html('<tr><td colspan="5" class="text-center py-4 text-danger">Tidak ada barang pada faktur ini.</td></tr>');
                    $('#btn-submit-retur').prop('disabled', true);
                    return;
                }
