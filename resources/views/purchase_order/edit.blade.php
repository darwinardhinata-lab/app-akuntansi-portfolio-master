...
@section('top_bar_left')
    ...
    <x-breadcrumb :links="[__('erp.bc_purchasing') => '#', __('erp.bc_purchase_orders') => route('po.index'), __('erp.bc_edit') => null]" />
@endsection
...
<div class="container-fluid mx-auto mt-4 mb-5" style="max-width: 1400px;">
    <form action="{{ route('po.update', $po->id) }}" method="POST" id="form-po">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h4 class="fw-bold mb-0 text-dark">{{ __('erp.edit_po') }}</h4>
            <div>
                    ...
                    <div class="card-body">
                        <div class="row g-3">
                            <div class="col-md-4">
                                ...
                                <input type="text" name="po_number" class="form-control fw-bold text-primary" value="{{ $po->po_number }}" required>
                            </div>
                            <div class="col-md-4">
                    ...
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="table table-bordered align-middle m-0" style="font-size: 0.85rem;">
                                ...
                                <tbody id="baris-po">
                                    @foreach($po->details as $index => $det)
                                    <tr>
                                        <td><input type="text" name="details[{{ $index }}][item_code]" class="form-control form-control-sm fw-bold" value="{{ $det->item_code }}" required></td>
                                        <td><input type="text" name="details[{{ $index }}][description]" class="form-control form-control-sm" value="{{ $det->description }}"></td>
                                        <td><input type="number" name="details[{{ $index }}][price]" class="form-control form-control-sm text-end price-input" value="{{ (int)$det->price }}" min="0" required></td>
                                        <td><input type="number" name="details[{{ $index }}][qty]" class="form-control form-control-sm text-center qty-input" value="{{ $det->qty }}" min="1" required></td>
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="is_include_ppn" id="is_include_ppn" class="form-select form-select-sm fw-bold">
                                <option value="0" {{ $po->is_include_ppn == 0 ? 'selected' : '' }}>{{ __('erp.exclude_vat') }}</option>
                                <option value="1" {{ $po->is_include_ppn == 1 ? 'selected' : '' }}>{{ __('erp.include_vat') }}</option>
                            </select>
                        </div>
                        <div class="mb-3">
                            ...
                            <select name="tax_addition_id" id="tax_addition_id" class="form-select form-select-sm">
                                ...
                                @foreach($taxesAddition as $tax)
                                    <option value="{{ $tax->id }}" data-rate="{{ $tax->rate }}" {{ $po->tax_addition_id == $tax->id ? 'selected' : '' }}>
                                        {{ $tax->tax_name }} ({{ number_format($tax->rate, 0) }}%)
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-4">
                            ...
                            <select name="tax_deduction_id" id="tax_deduction_id" class="form-select form-select-sm">
                                ...
                                @foreach($taxesDeduction as $tax)
                                    <option value="{{ $tax->id }}" data-rate="{{ $tax->rate }}" {{ $po->tax_deduction_id == $tax->id ? 'selected' : '' }}>
                                        {{ $tax->tax_name }} ({{ number_format($tax->rate, 2) }}%)
                                    </option>
                                @endforeach
                            </select>
                        </div>
...
<script>
    let barisCount = {{ count($po->details) + 1 }};
