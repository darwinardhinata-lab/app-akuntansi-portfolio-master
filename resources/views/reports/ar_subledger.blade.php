...
<div class="container-fluid px-4 mt-4">
    ...
    <ul class="nav nav-tabs mb-4 border-bottom">
        <li class="nav-item">
            <a class="nav-link fw-bold px-4 {{ $tab == 'tagihan' ? 'active bg-primary text-white border-bottom-0' : 'text-muted' }}" 
               href="{{ route('reports.ar_subledger', ['tab' => 'tagihan', 'account_code' => $accountCode ?? '']) }}">
                <i class="fa-solid fa-file-invoice me-1"></i> {{ __('erp.unpaid_bill') }}
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link fw-bold px-4 {{ $tab == 'pembayaran' ? 'active bg-success text-white border-bottom-0' : 'text-muted' }}" 
               href="{{ route('reports.ar_subledger', ['tab' => 'pembayaran', 'account_code' => $accountCode ?? '']) }}">
                <i class="fa-solid fa-money-bill-wave me-1"></i> {{ __('erp.payment_history') }}
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link fw-bold px-4 {{ $tab == 'retur' ? 'active bg-danger text-white border-bottom-0' : 'text-muted' }}" 
               href="{{ route('reports.ar_subledger', ['tab' => 'retur', 'account_code' => $accountCode ?? '']) }}">
                <i class="fa-solid fa-boxes-return me-1"></i> {{ __('erp.sales_return') }}
            </a>
        </li>
    </ul>
    ...
    @if(isset($accountCode) && $accountCode)
        ...
        @endphp
        <div class="alert alert-info py-2 shadow-sm fw-bold small mb-3 d-flex justify-content-between align-items-center" style="border-radius: 8px;">
            <span><i class="fa-solid fa-filter me-2"></i> {{ __('erp.filter_active') }} <span class="badge bg-white text-dark fw-bold fs-6">{{ $accountCode }} - {{ $accountName }}</span></span>
            <a href="{{ route('reports.ar_subledger', ['tab' => $tab]) }}" class="btn btn-sm btn-outline-danger bg-white fw-bold">{{ __('erp.clear_filter') }} <i class="fa-solid fa-circle-xmark ms-1"></i></a>
        </div>
    @endif
    ...
    <div class="card shadow-sm border-0 rounded-3">
        <div class="table-responsive bg-white rounded-3">
            <table class="table table-hover align-middle mb-0 text-nowrap">
                ...
                <tbody>
                    @php $data = $tab == 'tagihan' ? $unpaidInvoices : ($tab == 'pembayaran' ? $payments : $returns); @endphp
                    ...
                    @forelse($data as $row)
                        <tr>
                            ...
                            @else
                                <td class="text-end pe-4 fw-bold {{ $tab == 'pembayaran' ? 'text-success' : 'text-danger' }}">
